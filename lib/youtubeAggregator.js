    ...
    parser.parseString(xml, (err, result) => {
      ...
      for (let i = 0; i < Math.min(entries.length, MIN_VIDEOS); i++) {
        ...
        if (entry['media:group']?.['media:content']) {
          ...
          videoId = videoId || (content?.$?.url?.match(/v=([a-zA-Z0-9_-]+)/)?.[1] || '');
        ...
        if (!videoUrl && entry.link) {
          const link = Array.isArray(entry.link) ? entry.link.find(l => l.$?.rel === 'alternate') : entry.link;
          videoUrl = link?.$?.href || (typeof link === 'string' ? link : '');
        }
